...
public:
    ...
    sc_clock clk{"clk", clk_period, 0.5, SC_ZERO_TIME, true};
    sc_signal<bool> rst{"rst"};
    axi::ace_initiator_socket<SOCKET_WIDTH> intor{"intor"};
#ifdef WITH_SCV
    scv4axi::ace_recorder_module<SOCKET_WIDTH> intor_ace_rec{"intor_ace_rec"};
    scv4axi::axi_recorder_module<SOCKET_WIDTH> intor_axi_rec{"intor_axi_rec"};
#endif
    ...
    , tgt_pe("tgt_pe", tgt) {
        ...
        tgt_pe.clk_i(clk);
#ifdef WITH_SCV
        intor(intor_ace_rec.tsckt);
        intor_ace_rec.isckt(Adapter1.target_socket);
        Adapter1.initiator_socket(intor_axi_rec.tsckt);
        intor_axi_rec.isckt(tgt);
#else
        intor(Adapter1.target_socket);
        Adapter1.initiator_socket(tgt);
#endif
    }
